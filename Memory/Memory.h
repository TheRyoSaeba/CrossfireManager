...
public:
	...
	*/
	...
	inline bool IsValidPointer(uint64_t address)
	{
		bool res = false;
	...
	*/
	...
	template <typename T>
	void WriteValueInRange(uintptr_t start, uintptr_t end, T value, bool verbose = true)
	{
		for (uintptr_t addr = start; addr < end; addr += sizeof(T))
		{
			Write<T>(addr, value);
			if (verbose)
				printf("[WRITE] 0x%llX → %s\n", addr, std::to_string(value).c_str());
		}
	}
	template <typename T>
	void WriteValueInRange(const uintptr_t* addresses, size_t count, T value, bool verbose = true)
	{
		for (size_t i = 0; i < count; ++i)
		{
			Write<T>(addresses[i], value);
			if (verbose)
				printf("[WRITE] 0x%llX → %s\n", addresses[i], std::to_string(value).c_str());
		}
	}
