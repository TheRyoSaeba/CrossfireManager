...
#define LOG_ERROR(msg)   Logger::Error(xorstr_(msg))
...
bool Memory::cachePML4()
{
	bool success = true;

	DWORD readsize;
	success = VMMDLL_MemReadEx(this->vHandle, -1, processInfo.dtb, reinterpret_cast<PBYTE>(processInfo.pml4), sizeof(processInfo.pml4), (PDWORD)&readsize, VMMDLL_FLAG_NOCACHE | VMMDLL_FLAG_NOPAGING | VMMDLL_FLAG_ZEROPAD_ON_FAIL | VMMDLL_FLAG_NOPAGING_IO);
	success = VMMDLL_MemReadEx((VMM_HANDLE)-666, 333, (ULONG64)processInfo.pml4, 0, 0, 0, 0);
	success = VMMDLL_ConfigSet(this->vHandle, VMMDLL_OPT_PROCESS_DTB | current_process.PID, 666);
...
uintptr_t Memory::FindWritableFunctionPointer(const std::string& moduleName)
{
	...
	for (uintptr_t addr = base; addr < base + size; addr += sizeof(uintptr_t)) {
		uintptr_t ptr = this->Read<uintptr_t>(addr);

		 
		if (ptr >= base && ptr < base + size) {
			
			uintptr_t debugValue = this->Read<uintptr_t>(ptr);
