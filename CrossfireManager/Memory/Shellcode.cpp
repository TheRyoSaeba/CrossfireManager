...
bool c_shellcode::install_hook(void* target, void* detour, const std::string& process_name)
{
	int pid = mem.GetPidFromName(process_name);

	BYTE patch[14] = {
		0xFF, 0x25, 0x00, 0x00, 0x00, 0x00, // jmp [RIP + 0]
		0, 0, 0, 0, 0, 0, 0, 0             // the 64-bit address
	};

	*(uint64_t*)&patch[6] = (uint64_t)detour;

	if (!VMMDLL_MemWrite(mem.vHandle, pid, (uintptr_t)target, patch, sizeof(patch)))
	{
		LOG("[!] Failed to write permanent hook at 0x%p\n", target);
		return false;
	}

	LOG("[+] Permanent hook installed at 0x%p -> 0x%p\n", target, detour);
	return true;
}
