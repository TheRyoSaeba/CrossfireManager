...
void window::render_frame() {
    ...
    if (ESPDRAWBOX) {
        ESPFeatureManager::GetInstance().StartFeature(mem, gameState);
        ...
        ESPFeatureManager::GetInstance().StopFeature();
        {
...
void window::init(bool* running) {
    ...
    constexpr uintptr_t ltDrawPrimAddr = 0x140FB6110;
    KLASSES::LT_DRAWPRIM globalDrawPrim = mem.Read<KLASSES::LT_DRAWPRIM>(ltDrawPrimAddr);
    window::wx = globalDrawPrim.viewport.Width;
    window::wy = globalDrawPrim.viewport.Height;
    ...
    hwnd = CreateWindowExW(
        WS_EX_TOPMOST | WS_EX_LAYERED    | WS_EX_NOACTIVATE|WS_EX_LAYERED | WS_EX_TOOLWINDOW,  // Ensure transparency
